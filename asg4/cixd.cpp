...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (is) {
        ...
        outlog << "get : read file error: " << strerror (errno) << endl;
        header.command = cix_command::NAK;
        header.nbytes = errno;
        send_packet (client_sock, &header, sizeof(header));
        return;
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
    ...
    std::ofstream outfile(header.filename,ios::trunc|ios::out|ios::in);
    if (outfile) {
        ...
        outlog << "put : write file error: "<<strerror(errno) << endl;
        header.command = cix_command::NAK;
        header.nbytes = errno;
        send_packet (client_sock, &header, sizeof(header));
        return;
...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
    int ret = unlink(header.filename);
    if (ret == 0) {
        header.command = cix_command::ACK;
        outlog << "sending header " << header << endl;
        send_packet (client_sock, &header, sizeof(header));
        ...
        header.command = cix_command::NAK;
        header.nbytes = errno;
        outlog << "rm : rm file error: " << strerror (errno) << endl;
        outlog << "sending header " << header << endl;
        send_packet (client_sock, &header, sizeof(header));
    }
}
